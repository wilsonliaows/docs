...
3. Select `Another AWS Account`. Input Workato's Amazon S3 `Account ID` (**353360065216**). Type in an `External ID` and record this down, you will need to put this in the connection setting when setting up Amazon S3 connection in Workato.
...
7. Copy the `Role ARN`. You will need you will need to put this in the connection setting when setting up Amazon S3 connection in Workato.
