...
## How it works
This action takes a CSV file then create or update Quick Base records in bulk. Note that this action is not an Upsert operation. The way it works depend on your Quick Base table configuration, which is explained in detail below.

### Create records vs. Create & Update records
This action can either create new Quick Base records only, or create & update Quick Base records at the same time. The way to control this behaviour depends on your Quick Base table configuration:

#### If you Quick Base table uses default Record ID
![Quick Base Default Record ID](/assets/images/connectors/quick-base/qb-default-record-id.png)

Default `Record ID#` is automatically generated by Quick Base. Follow these instructions to create or update records:
