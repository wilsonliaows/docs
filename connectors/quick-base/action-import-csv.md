...
In the example above, this Quick Base table uses `Order ID` as the custom record ID, instead of the default `Record ID#`.

Custom record ID is not automatically generated by Quick Base, so you always need to provide it as a column in your CSV file. In this case, the action will always **Create & update records**.
