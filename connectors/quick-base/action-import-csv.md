...
Custom key field is not automatically generated by Quick Base, so you always need to provide it as a column in your CSV file. In this case, the action will **Create & update records**.
...
Note that if you use `Custom merge field` in `Quick Base table settings` (as shown below), Quick Base will only `update` records and not `create` records.
