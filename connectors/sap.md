...
There are 2 ways to create a connection in Workato. Click on `App Connections` on the navigation bar and then on `Create a new connection`; or click on the `Connection` tab in a recipe with SAP triggers/actions. Then, follow the steps below:
