...
## in_timezone
This function converts a timestamp expressed in a certain timezone to be expressed in another timezone. Refer to the following table for the timezone name to use in the formula.
...
## Changing displayed format of date via strftime
...
As shown above, each code (%B, %e, %I etc.) refers to a specific element of datetime. Static text and punctuation can also be added, such as commas (,), slashes (/), and colons (:). Here's a list of commonly used codes in Workato and what they return when we use the datetime value of `2017-06-05T17:13:27+00:00`.
